/*
lab 5, 6. USART, STRING INPUT
Name: Alexander Nunez
Class #: 11697
PI Name: Chris Crary
Description:   This program configure the appropriate USART module to  input a character string of arbitrary length. 
 */ 
...
MAIN:
	...
	mov R18, R17
	; point y to preallocated data memory
	ldi YL, low(STRING)	
	ldi YH, high(STRING)
	; point Z to preallocated data memory
	ldi ZL, low(STRING)	
	ldi ZH, high(STRING)
	rcall IN_STRING	; call in_string subroutine
	rcall OUT_STRING1	; call new out_string subroutine
DONE:
...
; FUNCTION:    receive a character string of arbitrary length with the relevant USART module 
; and store the received character string to some memory location(s) within data memory via the Y index.
; INPUT:        Receive the data.
; OUTPUT:       Received data is in program memory.
; DESTROYS:     None.
; REGS USED:	USARTD0_STATUS, USARTD0_DATA, r18
; CALLS:        IN_CHAR
...
IN_STRING:
	...
	cpi R16, CR
	brne CHECK
	ldi R16, 0xff	; r16 = null
	ldi R17, 0x01
CHECK:
	; check if char is backspace
	cpi R16, BS
	brne STORE
	;  when a backspace character is found, 
	dec YL	; decrement the Y index to allow for another character to be written
	inc R17
	rjmp IN_STRING
STORE:
	st Y+, R16	; store the character in the next appropriate data memory location with the Y index
	dec R17
	cpi R17, 0
	BRNE IN_STRING
LEAVE:
...
; *********************************************
; SUBROUTINE:   OUT_STRING1
; FUNCTION:    output a character string stored in data memory, 
; using the appropriate USART module.
; INPUT:        Data to be transmitted is in data memory .
; OUTPUT:       Transmit the data.
; DESTROYS:     None.
; REGS USED:	USARTD0_STATUS, USARTD0_DATA
; CALLS:        OUT_CHAR
...
OUT_STRING1:
	...
	rcall OUT_CHAR	; call the subroutine OUT_CHAR
	dec R18
	cpi r18, 0x00	; check if string 
	BREQ EXIT1	; exit subroutine if 
	rjmp OUT_STRING1	; loop to check next char
EXIT1:
	; when entire string is echoed, return from the subroutine
	ret