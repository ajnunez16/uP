 ...
 * lab8_2.c
 *
 * Created: 7/30/2019 6:09:45 PM
 *  Author: Aj
  this program will generate a sine wave of at least 256 data points, 
  with voltages ranging between 0V and AREFB, and with a frequency of 500 Hz. 
  Your program must utilize a built-in DAC and TC system, and interrupts must be used to manage timing.
 */ 

#include <avr/io.h>
#include <avr/interrupt.h>

#define PER ((2000000/1024) / 500)	// 500 Hz
#define MAX_VAL 255

int counter = 0;
 uint16_t arr[] = { 0x800,0x832,0x864,0x896,0x8c8,0x8fa,0x92c,0x95e,
...
// init dac to range in voltage from 0V to AREFB
void dac_init( void ){
		// ch0 trigger set by event system
		// DACA_CTRLB = DAC_CH0TRIG_bm;
		// use AREFB	
...
int main(void)
{
	...
	*/
	...
	PORTA_DIRSET = PIN6_bm | PIN5_bm;
	while(1){
		// check if data register is empty
		while( !( DACA_STATUS & DAC_CH0DRE_bm) );
		// loop that loads array of values for sine wave
		DACA_CH0DATA = arr[counter];
		DACA_CH0DATAH = (arr[counter] >> 8); // shifted 8 bits
	}
	return 0;	
}